// See https://vulkan-tutorial.com/en/Texture_mapping/Image_view_and_sampler
...
// Background color
const VkClearValue CLEAR_COLOR = {{{0.02f, 0.01f, 0.02f, 1.0f}}};
...
private:
    ...
    void initWindow(int width, int height) {
        ...
        m_window = glfwCreateWindow(width, height, "Vexora Engine", nullptr, nullptr);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        // flip the y axis to see upright
        ubo.proj[1][1] *= -1;
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Vexora Engine";
    ...
    void createLogicalDevice() {
        ...
        float queuePriority = 1.0f; // between 0.0 and 1.0, even if we have only 1 queue
    ...
    void createSwapChain() {
        ...
        // change if image needs to be flipped or rotated some way
        createInfo.preTransform = swapChainSupport.capabilities.currentTransform;

        // Dont blend alpha with other windows
    ...
    void createDescriptorSetLayout() {
        ...
        auto result = vkCreateDescriptorSetLayout(m_logicalDevice, &layoutInfo, nullptr, &m_descriptorSetLayout);
        if (result != VK_SUCCESS) {
    ...
    void createGraphicsPipeline() {
        ...
        // change for wireframe or points: ...MODE_LINE / ...MODE_POINT (needs extra GPU feature)
        rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // default: VK_POLYGON_MODE_FILL
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); i++) {
            ...
            auto result = vkCreateFramebuffer(m_logicalDevice, &framebufferInfo, nullptr, &m_swapChainFramebuffers[i]);
            if (result != VK_SUCCESS) {
    ...
    void createImage(uint32_t width, uint32_t height, 
        ...
        auto result = vkCreateImage(m_logicalDevice, &imageInfo, nullptr, &image);
        if (result != VK_SUCCESS) {
        ...
        vkBindImageMemory(m_logicalDevice, image, textureImageMemory, 0);
    }
    ...
    void createDescriptorPool() {
        ...
        poolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
         ...
         auto result = vkCreateDescriptorPool(m_logicalDevice, &poolInfo, nullptr, &m_descriptorPool);
        if (result != VK_SUCCESS) {
    ...
    void createDescriptorSets() {
        ...
        auto result = vkAllocateDescriptorSets(m_logicalDevice, &allocInfo, m_descriptorSets.data());
        if (result != VK_SUCCESS) {
    ...
    void createCommandBuffers() { 
        ...
        auto result = vkAllocateCommandBuffers(m_logicalDevice, &allocInfo, m_commandBuffers.data());
        if (result != VK_SUCCESS) {
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            auto result1 = vkCreateSemaphore(m_logicalDevice, &semaphoreInfo, nullptr, &m_imageAvailableSemaphores[i]);
            auto result2 = vkCreateSemaphore(m_logicalDevice, &semaphoreInfo, nullptr, &m_renderFinishedSemaphores[i]);
            auto result3 = vkCreateFence(m_logicalDevice, &fenceInfo, nullptr, &m_inFlightFences[i]);
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) {
        ...
        auto result = vkCreateShaderModule(m_logicalDevice, &createInfo, nullptr, &shaderModule);
        if (result != VK_SUCCESS) {
